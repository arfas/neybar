...
import { MapPin, Shield, Users, Zap, Check, X, Mail, ArrowRight, TrendingUp, Eye, Play, Pause, RotateCcw, UserPlus } from 'lucide-react';
...
const NayberSignupPage = () => {
  ...
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyF9hIgBimiEgelorpjYHlwgcy3tR7D6gZ7qsVR9kXNi9PPYPhblLGP49xl2WdhcXos/exec';
  ...
  useEffect(() => {
    if (isDemoPlaying && demoScene < 2) {
      const timer = setTimeout(() => setDemoScene(prev => prev + 1), 3000);
      return () => clearTimeout(timer);
    } else if (isDemoPlaying && demoScene === 2) {
  ...
  const demoScenes = [DemoScene1, DemoScene2, DemoScene3];
  const sceneNames = ["Distance-First", "Multiple Profiles", "Smart Matching"];
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="py-16 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">See Nayber in Action</h2>
            <p className="text-xl text-gray-600">Watch how it works in under 30 seconds</p>
          </div>
            ...
            <div className="aspect-video bg-white rounded-xl overflow-hidden relative">
              {!showDemo ? (
                <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-500 to-purple-600">
                  <button onClick={() => { setShowDemo(true); setIsDemoPlaying(true); setDemoScene(0); }} className="group">
                ...
                <>
                  {React.createElement(demoScenes[demoScene])}
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/80 backdrop-blur rounded-full px-6 py-3 flex items-center gap-4">
                    <button onClick={() => { setDemoScene(0); setIsDemoPlaying(true); }} className="text-white hover:text-blue-400 transition">
                      <RotateCcw className="w-4 h-4" />
                    </button>
                    <button onClick={() => setIsDemoPlaying(!isDemoPlaying)} className="w-10 h-10 bg-blue-500 hover:bg-blue-600 rounded-full flex items-center justify-center text-white transition">
                      {isDemoPlaying ? <Pause className="w-5 h-5" /> : <Play className="w-5 h-5 ml-0.5" />}
                    </button>
                    <div className="flex gap-2">
                      {demoScenes.map((_, i) => (
                        <button key={i} onClick={() => { setDemoScene(i); setIsDemoPlaying(false); }} className={`h-2 rounded-full transition ${i === demoScene ? 'bg-blue-500 w-8' : 'bg-gray-400 w-2'}`} />
                      ))}
                    </div>
                  </div>
      ...
      <div className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-xl text-gray-600 mb-12 text-center">The hyperlocal social network built for real neighborhoods</p>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-8 border-2 border-blue-200">
              ...
              <p className="text-gray-700 mb-4">See posts from people on your actual block, then nearby streets. Distance shown on every post.</p>
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-8 border-2 border-green-200">
              ...
              <p className="text-gray-700 mb-4">Get notified when someone with shared interests joins your block. Connect with people you will actually vibe with.</p>
              <div className="bg-white rounded-lg p-3 text-sm">
            ...
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-8 border-2 border-orange-200">
              ...
              <p className="text-gray-700 mb-4">Hire your neighbor to walk your dog, fix your fence, or babysit. All vetted, all hyperlocal.</p>
              <div className="bg-white rounded-lg p-3 text-sm">
      ...
      <div id="signup-section" className="bg-gradient-to-br from-blue-500 to-purple-600 py-16 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-3xl p-8 md:p-12 shadow-2xl">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3 text-center">Join the Waitlist</h2>
            <p className="text-gray-600 mb-8 text-center">Be among the first to experience Nayber in your neighborhood</p>
            {showSuccess ? (
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@email.com" className="w-full pl-12 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none text-lg" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={zipCode} onChange={(e) => setZipCode(e.target.value.replace(/\D/g, '').slice(0, 5))} placeholder="12345" maxLength={5} className="w-full pl-12 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none text-lg" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-3">Interests (Optional)</label>
                  <div className="flex flex-wrap gap-2">
                    {interests.map((interest) => (
                      <button key={interest} type="button" onClick={() => toggleInterest(interest)} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${selectedInterests.includes(interest) ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                        {interest}
                      </button>
                    ))}
                  </div>
                </div>
                <button onClick={handleSubmit} disabled={!email || zipCode.length !== 5} className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white font-bold py-4 rounded-xl hover:opacity-90 transition text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                  Get Early Access
                </button>
                <p className="text-xs text-gray-500 text-center">We will never spam you. Unsubscribe anytime.</p>
              </div>
            )}
      ...
      <div className="py-16 px-4 bg-white">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-xl text-gray-600 mb-8">Join {signups}+ people waiting for launch</p>
          <button onClick={() => scrollToSignup('footer')} className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-bold px-8 py-4 rounded-xl hover:opacity-90 transition text-lg shadow-lg">
            Get Early Access
            <ArrowRight className="w-5 h-5" />
          </button>
        </div>
      ...
      <div className="bg-gray-900 text-white py-8 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="mt-4 text-xs text-gray-500">2025 Nayber. All rights reserved.</div>
        </div>
      </div>
    </div>
  );
