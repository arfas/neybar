...
import DemoDistanceScene from './DemoDistanceScene';
import DemoProfileScene from './DemoProfileScene';
import DemoMatchingScene from './DemoMatchingScene';
...
const NayberSignupPage = () => {
  ...
  const handleSubmit = () => {
    if (email && zipCode && zipCode.length === 5) {
      console.log('Signup:', { email, zipCode, interests: selectedInterests, time: new Date().toISOString() });
